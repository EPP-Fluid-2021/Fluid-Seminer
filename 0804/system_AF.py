...
def compute(Q):
    ...
    QTMP = np.zeros([jxmax,jymax,4]) #x方向終了後の値を格納
    I=np.eye(4)
    ...
    for n in range(nmax):
        ...
        for k in range(jymax): #kごとにLU分解
            DeltaQ=np.zeros([jxmax,4])#LUの第一スイープ終了後
            

            Ap = np.zeros([jxmax,4,4])#A+
            Am = np.zeros([jxmax,4,4])#A-
            ...
            for i in range(1,jxmax):
                DeltaQ[i]=np.dot(np.linalg.inv(I+dt/dx*Ap[i]),G[i,k]+dt/dx*np.dot(Ap[i-1],DeltaQ[i-1]))
            for i in range(jxmax-2,-1,-1):
                QTMP[i,k]=np.dot(np.linalg.inv(I-dt/dx*Am[i]),DeltaQ[i]-dt/dx*np.dot(Am[i+1],QTMP[i+1,k]))
            
            
    
        for i in range(jxmax): #iごとにy方向LU分解
            DeltaQ=np.zeros([jymax,4])#LUの第一スイープ終了後
            DeltaQ2=np.zeros([jymax,4])

            Ap = np.zeros([jymax,4,4])#A+
            Am = np.zeros([jymax,4,4])#A-
            ...
            for k in range(jymax):
                ...
                Ap[k]=np.dot(Rp,np.dot(Lp,Rm))#行列の積で計算
                Am[k]=np.dot(Rp,np.dot(Lm,Rm))
            for k in range(1,jymax):
                DeltaQ[k]=np.dot(np.linalg.inv(I+dt/dy*Ap[k]),QTMP[i,k]+dt/dy*np.dot(Ap[k-1],DeltaQ[k-1]))
            for k in range(jymax-2,-1,-1):
                DeltaQ2[k]=np.dot(np.linalg.inv(I-dt/dy*Am[k]),DeltaQ[k]-dt/dy*np.dot(Am[k+1],DeltaQ2[k+1]))
